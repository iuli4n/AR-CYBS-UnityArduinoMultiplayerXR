...
namespace Microsoft.MixedReality.Toolkit.Editor
{
    ...
    public class BoundsControlInspector : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            if (target != null)
            {
                if (GUILayout.Button("CLEAN RIGROOTS"))
                {
                    string name = "rigRoot";
                    foreach (Transform t in ((BoundsControl)this.target).transform.GetComponentsInChildren<Transform>().Where(t => t.name == name).ToArray())
                    {
                        // make sure this is related to bounds
                        var c = t.gameObject.GetComponent<PointerHandler>();
                        if (c)
                        {
