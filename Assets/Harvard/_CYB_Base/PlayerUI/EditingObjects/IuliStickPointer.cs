...
namespace Microsoft.MixedReality.Toolkit.Input
{
    ...
    public class IuliStickPointer : IuliStickBaseControllerPointer, IMixedRealityNearPointer
    {
        public GameObject overrideGrabPoint;
        ...
        {
            if (overrideGrabPoint)
            {
                this.transform.position = overrideGrabPoint.transform.position;
                this.transform.rotation = overrideGrabPoint.transform.rotation;
                //Debug.LogWarning("** iuli override grab point");
            }
        ...
        public bool TryGetNearGraspPoint(out Vector3 result)
        {
            if (true)
            {
                result = overrideGrabPoint ? overrideGrabPoint.transform.position : Vector3.one * 9999f ;
                return true;
            }
            ...
            public bool TryUpdateQueryBufferForLayerMask(LayerMask layerMask, Vector3 pointerPosition, Vector3 pointerAxis, QueryTriggerInteraction triggerInteraction, bool ignoreCollidersNotInFOV)
            {
                using (TryUpdateQueryBufferForLayerMaskPerfMarker.Auto())
                {
                    ...
                    for (int i = 0; i < numColliders; i++)
                    {
                        ...
                        bool inAngle = true;/////IULIAN took out: Vector3.Dot(pointerAxis, relativeColliderPosition.normalized) >= Mathf.Cos(queryAngleRadians) || queryAngle >= 180.0f;
