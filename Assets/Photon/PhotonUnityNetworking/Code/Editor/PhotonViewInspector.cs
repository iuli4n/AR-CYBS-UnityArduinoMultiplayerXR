...
namespace Photon.Pun
{
    ...
    internal class PhotonViewInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            ///
            ///
            /// CUSTOM FROM HARVARD:CYB PROJECT
            /// CUSTOM FROM HARVARD:CYB PROJECT

            if (GUILayout.Button("CYB: Request Ownership"))
            {
                if (!this.m_Target.AmOwner)
                    this.m_Target.RequestOwnership();
            }

            /****
            if (this.m_Target.gameObject.GetComponentInChildren<TooltipTextValueController>())
            {
                TooltipTextValueController tvc = this.m_Target.gameObject.GetComponentInChildren<TooltipTextValueController>();
                {
                    String chan = tvc.myModel.Value;
                    tvc.myModel.Value = EditorGUILayout.TextField("Tooltip: ", tvc.myModel.Value);
                }
            }
            ****/

            /*
            if (this.m_Target.gameObject.GetComponentInChildren<SensorDataBroadcasterFromAtomic>())
            {
                SensorDataBroadcasterFromAtomic sdb = this.m_Target.gameObject.GetComponentInChildren<SensorDataBroadcasterFromAtomic>();
                if (sdb.sensorModels.Length > 0)
                {
                    String chan = sdb.sensorModels[0];
