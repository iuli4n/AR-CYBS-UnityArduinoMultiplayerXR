...
void SERIAL_CheckSerialMessages() {
  ...
  if (result.length() > 1) {
    //Serial.println("Arduino received: "+result+" ("+result.length()+")");
    ...
    if (result.startsWith(":")) {
      ...
      String v = result.substring(4); // PERF: shouldn't use a new string here
      int vi = v.toInt();
      
      // check if this substring is concatenated with others
      int spliti = v.indexOf(":");
      if (spliti >= 0) {
        // TODO: Sometimes we receive from Unity multiple things overlapped with each other, not sure why that can happen.
        
        Serial.println("ARDUINO ERROR - Received bad message: "+result);
